/************************************************************************
 * file:	LedEQSdk.h
 * brief:	Header file of 5E-6Q API
 * author:	fantx
 * date:	2018-12-18
 ***********************************************************************/
/*! ********************************************************************
...
#pragma pack (push,1)
	...
	}EQPicAreaSoundHeader_G6;//图文分区播放语音
...
* 注：这两个函数只能调用一次
******************************************************************/
LEDEQSDK_API int _CALL_STD InitSdk(void);
LEDEQSDK_API void _CALL_STD ReleaseSdk(void);
#endif
...
*     其他系列最大长度1K（1204）
******************************************************************/
LEDEQSDK_API int _CALL_STD set_packetLen(Ouint16 packetLen);                             
/*! ***************************************************************
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_searchController(Ping_data *retData);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_searchController(Ping_data *retData, Oint8* uartPort);
...
* 注：   针对 6代卡 的网络搜索命令
***************************************************************************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_search_Net_6G(NetSearchCmdRet *retData, Oint8* uartPort, Ouint8 nBaudRateType );
LEDEQSDK_API int _CALL_STD cmd_uart_search_Net_6G_Web(NetSearchCmdRet_Web *retData, Oint8* uartPort, Ouint8 nBaudRateType);



/*! ***************************************************************
* 函数名：       cmd_uart_ofsFormat（）
* 参数名：uartPort：串口端口号， baudRate：波特率
* 返回值：0 成功， 其他值为错误号
* 功 能：文件系统格式化
 ******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsFormat(Oint8* uartPort, Ouint8 baudRate);
...
* 发送节目前调用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsStartFileTransf(Oint8* uartPort, Ouint8 baudRate);
...
* 发送节目后调用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsEndFileTransf(Oint8* uartPort, Ouint8 baudRate);

/*! ***************************************************************
* 函数名：       cmd_uart_ofsDeleteFormatFile（）
* 参数名：uartPort：串口端口号， baudRate：波特率
*	fileNub:要删除的文件个数
*	fileName：要删除的文件名
* 返回值：0 成功， 其他值为错误号
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsDeleteFormatFile(Oint8* uartPort, Ouint8 baudRate, short fileNub, Ouint8 *fileName); 


/*! ***************************************************************
* 函数名：       cmd_uart_confDeleteFormatFile（）
* 参数名：uartPort：串口端口号， baudRate：波特率
*	fileNub:要删除的文件个数
*	fileName：要删除的文件名
* 返回值：0 成功， 其他值为错误号
...
* fileName是4个字节 fileNub值为N就要把N个fileName拼接 fileName大小 = fileName（4byte）*N
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_confDeleteFormatFile(Oint8* uartPort, Ouint8 baudRate, short fileNub, Ouint8 *fileName); 


/*! ***************************************************************
* 函数名：       cmd_uart_ofsGetMemoryVolume（）
* 参数名：uartPort：串口端口号， baudRate：波特率
...
* 发节目前需要查询防止空间不够用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsGetMemoryVolume(Oint8* uartPort, Ouint8 baudRate, Ouint32 *totalMemVolume, Ouint32 *availableMemVolume);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsWriteFile(Oint8* uartPort, Ouint8 baudRate, Ouint8 *fileName, Ouint8 fileType, Ouint32 fileLen, Ouint8 overwrite, Ouint8 *fileAddre);
...
* 内部包含多条命令注意返回状态方便查找问题
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_confWriteFile(Oint8* uartPort, Ouint8 baudRate, Ouint8 *fileName, Ouint8 fileType, Ouint32 fileLen, Ouint8 overwrite, Ouint8 *fileAddre);

/*! ***************************************************************
* 函数名：       cmd_ofsStartReedFile（）
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsStartReedFile(Oint8* uartPort, Ouint8 baudRate, Ouint8 *fileName, Ouint32* fileSize, Ouint32 *fileCrc);

/*! ***************************************************************
* 函数名：       cmd_uart_confStartReedFile（）
* 参数名：uartPort：串口端口号， baudRate：波特率
*	fileName：需要读取的文件名
*	fileSize：回读文件大小
*	fileCrc：回读的文件CRC
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_confStartReedFile(Oint8* uartPort, Ouint8 baudRate, Ouint8 *fileName, Ouint32* fileSize, Ouint32 *fileCrc);


/*! ***************************************************************
* 函数名：       cmd_uart_ofsReedFileBlock（）
* 参数名：uartPort：串口端口号， baudRate：波特率
*	fileName：需要读取的文件名
*	fileAddre：传入读文件写的位置
* 返回值：0 成功， 其他值为错误号
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsReedFileBlock(Oint8* uartPort, Ouint8 baudRate, Ouint8 *fileName, Ouint8* fileAddre);

/*! ***************************************************************
* 函数名：       cmd_uart_confReedFileBlock(）
	* 参数名：uartPort：串口端口号， baudRate：波特率
*	fileName：需要读取的文件名
*	fileAddre：传入读文件写的位置
* 返回值：0 成功， 其他值为错误号
...
* fileAddre大小根据cmd_ofsStartReedFile函数回调值确定
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_confReedFileBlock(Oint8* uartPort, Ouint8 baudRate, Ouint8 *fileName, Ouint8* fileAddre);

/*! ***************************************************************
* 函数名：       cmd_uart_ofsReedDirBlock（）
* 参数名：uartPort：串口端口号， baudRate：波特率
*	fileName：需要读取的文件名
*	fileAddre：传入读文件写的位置
* 返回值：0 成功， 其他值为错误号
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsReedDirBlock(Oint8* uartPort, Ouint8 baudRate, GetDirBlock_G56 *dirBlock);

/*! ***************************************************************
* 函数名：  cmd_ofs_freeDirBlock（）
* 参数名：
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_ofsFreeDirBlock(GetDirBlock_G56 *dirBlock);

LEDEQSDK_API int _CALL_STD cmd_uart_ofsGetTransStatus(Oint8* uartPort, Ouint8 baudRate, Ouint8 *r_w, Ouint8* fileName, Ouint32 *fileCrc, Ouint32 *fileOffset);


/*! ***************************************************************
* 函数名：       cmd_uart_sendConfigFile(）
* 参数名：uartPort：串口端口号， baudRate：波特率
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_sendConfigFile(Oint8* uartPort, Ouint8 baudRate, ConfigFile *configData);

/*! ***************************************************************
	* 函数名：       cmd_uart_programLock（）
	* 参数名：uartPort：串口端口号， baudRate：波特率
	...
	* 具体使用方法参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_uart_programLock(Oint8* uartPort, Ouint8 baudRate, Ouint8 nonvolatile, Ouint8 lock, Ouint8 *name, Ouint32 lockDuration);


LEDEQSDK_API int _CALL_STD cmd_uart_programLock_6G(Oint8* uartPort, Ouint8 baudRate, Ouint8 nonvolatile, Ouint8 lock, Ouint8 *name, Ouint32 lockDuration);
...
* 通讯方式（UDP
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_AT_setWifiSsidPwd(Ouint8* ssid, Ouint8* pwd);
...
* 通讯方式（UDP）
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_AT_getWifiSsidPwd(Ouint8* ssid, Ouint8* pwd);
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_udpNetworkSearch(heartbeatData *retData); //网络搜索
...
* 注：    针对 6代卡 的网络搜索命令
***********************************************************************************************************************/
LEDEQSDK_API int _CALL_STD cmd_udpNetworkSearch_6G(NetSearchCmdRet *retData); 
LEDEQSDK_API int _CALL_STD cmd_udpNetworkSearch_6G_Web(NetSearchCmdRet_Web *retData); 
...
* 此命令用来搜索加屏使用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_udpPing(Ping_data *retData); //UDP ping命令并返回IP地址
...
* 需要修改MAC地址的时候调用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_udpSetMac(Ouint8 *mac); 
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_udpSetIP(Ouint8 mode, Ouint8 *ip, Ouint8 *subnetMask, Ouint8 *gateway, short port, Ouint8 serverMode, Ouint8 *serverIP, short serverPort, Ouint8 *password, short heartbeat, Ouint8 *netID);// 由于传入参数到内部都需要转换没有使用结构体
...
* 此命令调用后所有参数全部会丢失
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_sysReset(Ouint8* ip, Ouint16 port);
...
* 和UDP PING命令获取到的参数相同
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_tcpPing(Ouint8* ip, Ouint16 port, Ping_data *retData);
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_check_time(Ouint8* ip, Ouint16 port);
...
* 注：   针对 6代卡 的网络搜索命令
***********************************************************************************************************************/
LEDEQSDK_API int _CALL_STD cmd_tcpNetworkSearch_6G(Ouint8* ip, Ouint16 port, NetSearchCmdRet *retData);
LEDEQSDK_API int _CALL_STD cmd_tcpNetworkSearch_6G_Web(Ouint8* ip, Ouint16 port, NetSearchCmdRet_Web *retData);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_coerceOnOff(Ouint8* ip, Ouint16 port, Ouint8 onOff);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_timingOnOff(Ouint8* ip, Ouint16 port, Ouint8 groupNum, Ouint8 *data);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_cancelTimingOnOff(Ouint8* ip, Ouint16 port);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setBrightness(Ouint8* ip, Ouint16 port, Brightness *brightness);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_readControllerID(Ouint8* ip, Ouint16 port, Ouint8 *ControllerID);
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_screenLock(Ouint8* ip, Ouint16 port, Ouint8 nonvolatile, Ouint8 lock);
...
/*! ***************************************************************
	...
	* 具体使用方法参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_programLock(Ouint8* ip, Ouint16 port,  Ouint8 nonvolatile, Ouint8 lock, Ouint8 *name, Ouint32 lockDuration);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_check_controllerStatus(Ouint8* ip, Ouint16 port,  ControllerStatus_G56 *controllerStatus);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setPassword(Ouint8* ip, Ouint16 port, Ouint8 *oldPassword, Ouint8 *newPassword);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_deletePassword(Ouint8* ip, Ouint16 port, Ouint8 *password);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setDelayTime(Ouint8* ip, Ouint16 port, short delayTime);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setBtnFunc(Ouint8* ip, Ouint16 port, Ouint8 btnMode);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setTimingReset(Ouint8* ip, Ouint16 port, TimingReset *cmdData);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setDispMode(Ouint8* ip, Ouint16 port, Ouint8 dispMode);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_battieTime(Ouint8* ip, Ouint16 port, Ouint8 mode, BattleTime *battieData);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_getStopwatch(Ouint8* ip, Ouint16 port, Ouint8 mode, Ouint32 *timeValue);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_getSensorBrightnessValue(Ouint8* ip, Ouint16 port, Ouint32 *brightnessValue);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setSpeedAdjust(Ouint8* ip, Ouint16 port, short speed);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_setScreenAddress(Ouint8* ip, Ouint16 port, short address);
...
* 具体细节参考协议
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsFormat(Ouint8* ip, Ouint16 port);
...
* 发送节目前调用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsStartFileTransf(Ouint8* ip, Ouint16 port);
...
* 发送节目后调用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsEndFileTransf(Ouint8* ip, Ouint16 port);
...
* fileName是4个字节 fileNub值为N就要把N个fileName拼接 fileName大小 = fileName（4byte）*N
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsDeleteFormatFile(Ouint8* ip, Ouint16 port, short fileNub, Ouint8 *fileName); 


/*! ***************************************************************
* 函数名：       cmd_confDeleteFormatFile（）
* 参数名：ip：控制器IP， port：控制器端口
*	fileNub:要删除的文件个数
*	fileName：要删除的文件名
* 返回值：0 成功， 其他值为错误号
...
* fileName是4个字节 fileNub值为N就要把N个fileName拼接 fileName大小 = fileName（4byte）*N
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_confDeleteFormatFile(Ouint8* ip, Ouint16 port, short fileNub, Ouint8 *fileName); 


/*! ***************************************************************
* 函数名：       cmd_ofsGetMemoryVolume（）
* 参数名：ip：控制器IP， port：控制器端口
*	totalMemVolume：全部空间大小
*	availableMemVolume：剩余空间大小
* 返回值：0 成功， 其他值为错误号
...
* 发节目前需要查询防止空间不够用
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsGetMemoryVolume(Ouint8* ip, Ouint16 port, Ouint32 *totalMemVolume, Ouint32 *availableMemVolume);
...
* 内部包含多条命令注意返回状态方便查找问题
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsWriteFile(Ouint8* ip, Ouint16 port, Ouint8 *fileName, Ouint8 fileType, Ouint32 fileLen, Ouint8 overwrite, Ouint8 *fileAddre);
...
* 内部包含多条命令注意返回状态方便查找问题
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_confWriteFile(Ouint8* ip, Ouint16 port, Ouint8 *fileName, Ouint8 fileType, Ouint32 fileLen, Ouint8 overwrite, Ouint8 *fileAddre);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsStartReedFile(Ouint8* ip, Ouint16 port, Ouint8 *fileName, Ouint32* fileSize, Ouint32 *fileCrc);

/*! ***************************************************************
* 函数名：       cmd_confStartReedFile（）
* 参数名：ip：控制器IP， port：控制器端口
*	fileName：需要读取的文件名
*	fileSize：回读文件大小
*	fileCrc：回读的文件CRC
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_confStartReedFile(Ouint8* ip, Ouint16 port, Ouint8 *fileName, Ouint32* fileSize, Ouint32 *fileCrc);


/*! ***************************************************************
* 函数名：       cmd_ofsReedFileBlock（）
* 参数名：ip：控制器IP， port：控制器端口
*	fileName：需要读取的文件名
*	fileAddre：传入读文件写的位置
* 返回值：0 成功， 其他值为错误号
...
* fileAddre大小根据cmd_ofsStartReedFile函数回调值确定
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsReedFileBlock(Ouint8* ip, Ouint16 port, Ouint8 *fileName, Ouint8* fileAddre);

/*! ***************************************************************
* 函数名：       cmd_confReedFileBlock(）
	* 参数名：ip：控制器IP， port：控制器端口
*	fileName：需要读取的文件名
*	fileAddre：传入读文件写的位置
* 返回值：0 成功， 其他值为错误号
...
* fileAddre大小根据cmd_ofsStartReedFile函数回调值确定
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_confReedFileBlock(Ouint8* ip, Ouint16 port, Ouint8 *fileName, Ouint8* fileAddre);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofsReedDirBlock(Ouint8* ip, Ouint16 port, GetDirBlock_G56 *dirBlock);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_getFileAttr(GetDirBlock_G56 *dirBlock, Ouint16 number, FileAttribute_G56 *fileAttr);
...
* dirBlock 上述两条命令调用完成后dirBlock不再使用时用此函数释放文件列表
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_ofs_freeDirBlock(GetDirBlock_G56 *dirBlock);

LEDEQSDK_API int _CALL_STD cmd_ofsGetTransStatus(Ouint8* ip, Ouint16 port, Ouint8 *r_w, Ouint8* fileName, Ouint32 *fileCrc, Ouint32 *fileOffset);


/*! ***************************************************************
* 函数名：       cmd_firmwareActivate（）
...
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_firmwareActivate(Ouint8* ip, Ouint16 port, Ouint8* firmwareFileName);



/*! ***************************************************************
* 函数名：       cmd_sendConfigFile(）
* 参数名：ip：控制器IP， port：控制器端口
		configData 请参考结构体ConfigFile
* 返回值：0 成功， 其他值为错误号
* 功 能：发送5代卡配置文件到控制器
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_sendConfigFile(Ouint8* ip, Ouint16 port, ConfigFile *configData);

/*! ***************************************************************
* 函数名：       cmd_sendConfigFile_G6(）
* 参数名：ip：控制器IP， port：控制器端口
		configData 请参考结构体ConfigFile
* 返回值：0 成功， 其他值为错误号
* 功 能：发送5代卡配置文件到控制器
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD cmd_sendConfigFile_G6(Ouint8* ip, Ouint16 port, ConfigFile_G6 *configData);
...
* 注：
******************************************************************/
LEDEQSDK_API int _CALL_STD get_crc16(FileCRC16_G56 *crc16);
...
* 注：
******************************************************************/
LEDEQSDK_API int _CALL_STD get_crc32(FileCRC32_G56 *crc32);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureArea(Ouint32 programID, Ouint8* ip, Ouint16 port);
...
* 三个参数请参考各自枚举值
******************************************************************/
LEDEQSDK_API int _CALL_STD program_setScreenParams_G56(E_ScreenColor_G56 color, Ouint16 ControllerType, E_DoubleColorPixel_G56 doubleColor); //设置屏相关属性

//LEDEQSDK_API int _CALL_STD program_setScreenParams_G6(E_ScreenColor_G56 color, Ouint16 ControllerType, E_DoubleColorPixel_G56 doubleColor);
...
******************************************************************/
LEDEQSDK_API int _CALL_STD program_addProgram(EQprogramHeader *programH); //添加节目句柄
/*! ***************************************************************
* 函数名：       program_changeProgramParams（）
*	programH：参考结构体EQprogramHeader
* 返回值：0 成功， 其他值为错误
* 功 能：修改已添加节目的一些参数
* 注：
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_changeProgramParams(EQprogramHeader *programH);
/*! ***************************************************************
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_deleteProgram();

/*! ***************************************************************
* 函数名：       program_freeBuffer(）
* 参数名：
*
* 返回值：0 成功， 其他值为错误号
* 功 能：五代协议释放生成节目文件的缓冲区
* 注：
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_freeBuffer(EQprogram* program);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_addPlayPeriodGrp(EQprogramppGrp_G56 *header);
/*! ***************************************************************
* 函数名：       program_AddArea（）
* 参数名：
*	areaID：区域的ID号
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_AddArea(Ouint16 areaID, EQareaHeader *aheader);//添加区域句柄
/*! ***************************************************************
* 函数名：       program_deleteArea（）
* 参数名：
*	areaID：区域的ID号
* 返回值：0 成功， 其他值为错误号
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_deleteArea(Ouint16 areaID);
/*! ***************************************************************
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_picturesAreaAddTxt(Ouint16 areaID, Ouint8* str, Ouint8* fontName, EQpageHeader* pheader);//画字符到区域
/*! ***************************************************************
* 函数名：       program_picturesAreaChangeTxt（）
*	areaID：区域的ID号
*	str：需要画的字符
*	pheader：参考结构体EQpageHeader
*
...
******************************************************************/
LEDEQSDK_API int _CALL_STD program_picturesAreaChangeTxt(Ouint16 areaID, Ouint8* str, EQpageHeader* pheader);
/*! ***************************************************************
* 函数名：       program_fontPath_picturesAreaAddTxt（）
*	areaID：区域的ID号
*	str：需要画的字符
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_fontPath_picturesAreaAddTxt(Ouint16 areaID, Ouint8* str, Ouint8* fontPathName, EQpageHeader* pheader);
/*! ***************************************************************
* 函数名：       program_fontPath_picturesAreaChangeTxt（）
*	areaID：区域的ID号
*	str：需要更换的字符串
*	pheader：参考结构体EQpageHeader
* 返回值：0 成功， 其他值为错误号
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_fontPath_picturesAreaChangeTxt(Ouint16 areaID, Ouint8* str, EQpageHeader* pheader);
/*! ***************************************************************
...
******************************************************************/
LEDEQSDK_API int _CALL_STD program_IntegrateProgramFile(EQprogram* program);
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_picturesAreaAddFrame(Ouint16 areaID,EQareaframeHeader* afHeader, Ouint8* picPath);
/*! ***************************************************************
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureAreaGetOnePage(Ouint16 areaID,Oint32 pageNum,getPageData* pageData);
/*! ***************************************************************
...
LEDEQSDK_API int _CALL_STD program_pictureAreaAddPic(Ouint16 areaID,Ouint16 picID,EQpageHeader* pheader,Ouint8* picPath);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_addFrame(EQscreenframeHeader* sfHeader,Ouint8* picPath);
/*! ***************************************************************
* 函数名：       program_changeFrame（）
*	EQscreenframeHeader：参考结构体EQscreenframeHeader
*	picPath：边框图片路径
* 返回值：0 成功， 其他值为错误号
* 功 能：节目修改已添加边框的一些参数
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_changeFrame(EQscreenframeHeader* sfHeader,Ouint8* picPath);
/*! ***************************************************************
* 函数名：       program_removeFrame（）
*	
*
* 返回值：0 成功， 其他值为错误号
* 功 能：节目去掉边框
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_removeFrame();
/*! ***************************************************************
* 函数名：       program_pictureAreaRemoveFrame（）
*	areaID：区域的ID号
*
* 返回值：0 成功， 其他值为错误号
* 功 能：区域去掉边框
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureAreaRemoveFrame(Ouint16 areaID);

/*! ***************************************************************
* 函数名：       program_MoveArea()
*	areaID：区域的ID号
*   x:区域left坐标
*   y:区域top坐标
*   width:区域宽度
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_MoveArea(Ouint16 areaID,Oint32 x,Oint32 y,Oint32 width,Oint32 height);

/*! ***************************************************************
* 函数名：       program_timeAreaAddContent()
*	areaID：区域的ID号
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaAddContent(Ouint16 areaID,EQtimeAreaData_G56* timeData);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_fontPath_timeAreaAddContent(Ouint16 areaID,EQtimeAreaData_G56* timeData);

/*! ***************************************************************
* 函数名：       program_timeAreaChangeContent()
*	areaID：区域的ID号
*   timeData:详情请见时间区数据格式结构体EQtimeAreaData_G56
*
* 返回值：0 成功， 其他值为错误号
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaChangeContent(Ouint16 areaID,EQtimeAreaData_G56* timeData);

/*! ***************************************************************
* 函数名：       program_timeAreaGetOnePage(）
* 参数名：
*	areaID：区域ID
*   pageNum: 第几页，从0开始计算
* 返回值：0 成功， 其他值为错误号
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaGetOnePage(Ouint16 areaID,getPageData* pageData);
/*! ***************************************************************
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaAddAnalogClock(Ouint16 areaID,EQAnalogClockHeader_G56 *header,E_ClockStyle cStyle, ClockColor_G56* cColor);
/*! ***************************************************************
* 函数名：       program_timeAreaChangeAnalogClock(）
* 参数名：
*	areaID：区域ID
*   header: 详情见EQAnalogClockHeader_G56结构体
*   cStyle: 表盘样式，详情见E_ClockStyle
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaChangeAnalogClock(Ouint16 areaID,EQAnalogClockHeader_G56 *header,E_ClockStyle cStyle, ClockColor_G56* cColor);
/*! ***************************************************************
* 函数名：       program_timeAreaChangeDialPic(）
* 参数名：
*	areaID： 区域ID
*   picPath: 表盘图片位置
* 返回值：0 成功， 其他值为错误号
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaChangeDialPic(Ouint16 areaID, Ouint8* picPath);


/*! ***************************************************************
* 函数名：       program_timeAreaChangeDialPicAdd_G56(）
* 参数名：
*	areaID： 区域ID
*   picPath: 表盘图片位置
* 返回值：0 成功， 其他值为错误号
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaChangeDialPicAdd_G56(Ouint16 areaID, Ouint8* picAdd, Ouint32 picLen);


/*! ***************************************************************
* 函数名：       program_timeAreaRemoveDialPic(）
* 参数名：
*	areaID：区域ID
* 返回值：0 成功， 其他值为错误号
* 功 能：时间分区移除外部添加的表盘图片
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaRemoveDialPic(Ouint16 areaID);
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaTxt_6G(Ouint8* pIP, Ouint32 nPort,  E_ScreenColor_G56 color, Ouint8 uAreaId, Ouint16 uAreaX, Ouint16 uAreaY, 
...
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaTxtDetails_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaId, EQareaHeader_G6* oAreaHeader_G6,
...
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaTxtDetails_WithProgram_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaId, EQareaHeader_G6* oAreaHeader_G6,
									EQpageHeader_G6* stPageHeader, Ouint8* fontName, Ouint8* strAreaTxtContent, Ouint16 RelateProNum, Ouint16* RelateProSerial );


//更新动态区图片：仅显示动态区;
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaPic_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaId, Ouint16	AreaX, Ouint16 AreaY, 
...
//RelateProNum = 0 时，关联所有节目，与所有节目一起播放，如果没有节目，则不播放该动态区；
//			   > 0 时, 指定关联节目，要关联的节目ID存放在RelateProSerial[]中；
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaPic_WithProgram_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaId, Ouint16 AreaX, Ouint16 AreaY, 
...
LEDEQSDK_API int _CALL_STD dynamicAreaS_AddTxtDetails_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams);

//同时更新多个动态区:并与节目关联，即与节目一起显示
//RelateProNum = 0 时，关联所有节目，与所有节目一起播放，如果没有节目，则不播放该动态区；
//			   > 0 时, 指定关联节目，要关联的节目ID存放在RelateProSerial[]中；
LEDEQSDK_API int _CALL_STD dynamicAreaS_AddTxtDetails_WithProgram_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams, Ouint16 RelateProNum, Ouint16* RelateProSerial );


//更新动态区图片：仅显示动态区;
LEDEQSDK_API int _CALL_STD dynamicAreaS_AddAreaPic_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams);


//动态区图片关联节目: 
//RelateProNum = 0 时，关联所有节目，与所有节目一起播放，如果没有节目，则不播放该动态区；
//			   > 0 时, 指定关联节目，要关联的节目ID存放在RelateProSerial[]中；
LEDEQSDK_API int _CALL_STD dynamicAreaS_AddAreaPic_WithProgram_6G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color, Ouint8 uAreaCount, DynamicAreaParams* pParams, Ouint16 RelateProNum, Ouint16* RelateProSerial);
...
uAreaId = 0xff:删除所有区域
*/
LEDEQSDK_API int _CALL_STD dynamicArea_DelArea_6G(Ouint8* pIP, Ouint32 nPort, Oint8 uAreaId);
/*
功能：删除多个动态区：
参数：
...
*/
LEDEQSDK_API int _CALL_STD dynamicArea_DelAreas_6G(Ouint8* pIP, Ouint32 nPort, Oint8 uAreaCount, Oint8* pAreaID);
...
*/
LEDEQSDK_API int _CALL_STD dynamicArea_InsertSoundIndepend(Ouint8* pIP, Ouint32 nPort, EQSoundDepend_6G stSoundData, Ouint8 VoiceFlg, Ouint8 StoreFlag);
...
*/
LEDEQSDK_API int _CALL_STD  dynamicArea_UpdateSoundIndepend(Ouint8* pIP, Ouint32 nPort, EQSoundDepend_6G* stSoundData, Ouint16 nSoundDataCount, Ouint8 StoreFlag);
...
*/
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaWithTxt_5G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color,
	Ouint8 uAreaId,
	Ouint8 RunMode,
	Ouint16 Timeout,
	Ouint8 RelateAllPro,
...
*/
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaWithPic_5G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color,
	Ouint8 uAreaId,
	Ouint8 RunMode,
	Ouint16 Timeout,
	Ouint8 RelateAllPro,
...
*/
LEDEQSDK_API int _CALL_STD dynamicArea_AddAreaInfos_5G(Ouint8* pIP, Ouint32 nPort, E_ScreenColor_G56 color,
	Ouint8 uAreaId,
	Ouint8 RunMode,
	Ouint16 Timeout,
	Ouint8 RelateAllPro,
...
*/
LEDEQSDK_API int _CALL_STD dynamicArea_DelArea_5G(Ouint8* pIP, Ouint32 nPort, Ouint8 uAreaId);

/*
功能：删除多个动态区：
*/
LEDEQSDK_API int _CALL_STD dynamicArea_DelAreaS_5G(Ouint8* pIP, Ouint32 nPort, Oint8 uAreaCount, Oint8* pAreaID);
...
//LEDEQSDK_API int _CALL_STD dynamicArea_addDynamicArea(Ouint16 areaID,EQdynamicHeader* header);
LEDEQSDK_API int _CALL_STD dynamicArea_deleteDynamicArea(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_addTxt(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_fontPath_addTxt(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_changeTxt(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_fontPath_changeTxt(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_addPic(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_changePic(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_addFrame(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_changeFrame(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_removeFrame(Ouint16 areaID);
LEDEQSDK_API int _CALL_STD dynamicArea_integrateDAreaFile();
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_addProgram_G6(EQprogramHeader_G6 *programH);


/*! ***************************************************************
* 函数名：       program_addPlayPeriodGrp_G6（）
* 返回值：0 成功， 其他值为错误
* 功 能：添加节目播放时段
* 注：
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_addPlayPeriodGrp_G6(EQprogramppGrp_G56 *header);
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_deleteProgram_G6();
/*! ***************************************************************
* 函数名：       program_freeBuffer_G6(）
* 参数名：
*
* 返回值：0 成功， 其他值为错误号
* 功 能：释放生成节目文件的缓冲区
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_freeBuffer_G6(EQprogram_G6* program);

/*! ***************************************************************
* 函数名：       program_changeProgramParams_G6（）
*	EQprogramHeader_G6：参考结构体EQprogramHeader_G6
* 返回值：0 成功， 其他值为错误号
* 功 能：修改已添加节目的一些参数
* 注：
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_changeProgramParams_G6(EQprogramHeader_G6 *programH);
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_addFrame_G6(EQscreenframeHeader_G6* sfHeader,Ouint8* picPath);
/*! ***************************************************************
* 函数名：       program_changeFrame_G6（）
*	sfHeader：参考结构体EQscreenframeHeader_G6
*	picPath：边框图片路径
* 返回值：0 成功， -1 不成功
* 功 能：节目修改已添加边框的一些参数
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_changeFrame_G6(EQscreenframeHeader_G6* sfHeader,Ouint8* picPath);
/*! ***************************************************************
* 函数名：       program_removeFrame_G6（）
* 返回值：0 成功
* 功 能：节目去掉边框
* 注：
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_removeFrame_G6();
/*! ***************************************************************
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_addArea_G6(Ouint16 areaID, EQareaHeader_G6 *aheader);
/*! ***************************************************************
* 函数名：       program_deleteArea_G6（）
* 参数名：
*   areaID：区域ID号
*
* 返回值：0 成功， 其他值为错误号
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_deleteArea_G6(Ouint16 areaID);

/*! ***************************************************************
* 函数名：       program_MoveArea_G6()
*	areaID：区域的ID号
*   x:区域left坐标
*   y:区域top坐标
*   width:区域宽度
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_MoveArea_G6(Ouint16 areaID,Oint32 x,Oint32 y,Oint32 width,Oint32 height);
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_picturesAreaAddTxt_G6(Ouint16 areaID, Ouint8* str, Ouint8* fontName,EQpageHeader_G6* pheader);
/*! ***************************************************************
* 函数名：       program_picturesAreaChangeTxt_G6（）
*	areaID：区域的ID号
*	str：需要画的文字
*	pheader：参考结构体EQpageHeader_G6
*
...
******************************************************************/
LEDEQSDK_API int _CALL_STD program_picturesAreaChangeTxt_G6(Ouint16 areaID, Ouint8* str, EQpageHeader_G6* pheader);
/*! ***************************************************************
* 函数名：       program_fontPath_picturesAreaAddTxt_G6（）
*	areaID：区域的ID号
*	str：需要画的文字
*	fontPathName：字体绝对路径加字库文件名称
*	pheader：参考结构体EQpageHeader_G6
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_fontPath_picturesAreaAddTxt_G6(Ouint16 areaID, Ouint8* str, Ouint8* fontPathName, EQpageHeader_G6* pheader);
/*! ***************************************************************
* 函数名：       program_fontPath_picturesAreaChangeTxt_G6（）
*	areaID：区域的ID号
*	str：需要画的文字
*	pheader：参考结构体EQpageHeader_G6
*
...
* 一定要参考协议对每一个值都不能理解出错否则发下去的内容显示肯定不是自己想要的
******************************************************************/
LEDEQSDK_API int _CALL_STD program_fontPath_picturesAreaChangeTxt_G6(Ouint16 areaID, Ouint8* str, EQpageHeader_G6* pheader);
/*! ***************************************************************
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureAreaAddPic_G6(Ouint16 areaID,Ouint16 picID,EQpageHeader_G6* pheader,Ouint8* picPath);
/*! ***************************************************************
* 函数名：       program_backGroundPic_G6（）
*	areaID：区域的ID号
*   picID：图片编号，从0开始，第一次添加图片为0，第二次添加图片为1，依次累加，每个id对应一张图片
*	EQpageHeader_G6：参考结构体EQpageHeader_G6
*	picPath：图片的绝对路径加图片名称
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_backGroundPic_G6(Ouint16 areaID,Ouint16 picID, EQpageHeader_G6* pheader, Ouint8* picPath);

/*! ***************************************************************
* 函数名：       program_backGroundColor_G6（）
*	areaID：区域的ID号
*   picID：图片编号，从0开始，第一次添加图片为0，第二次添加图片为1，依次累加，每个id对应一张图片
*	EQpageHeader_G6：参考结构体EQpageHeader_G6
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_backGroundColor_G6(Ouint16 areaID, EQpageHeader_G6* pheader, Ouint32 BGColor);


/*! **************************************************************** 函数名：       program_pictureAreaChangePic_G6（）
*	areaID：区域的ID号
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureAreaChangePic_G6(Ouint16 areaID,Ouint16 picID,EQpageHeader_G6* pheader,Ouint8* picPath);
/*! ***************************************************************
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureAreaEnableSound_G6(Ouint16 areaID,EQPicAreaSoundHeader_G6 sheader,Ouint8* soundData);
/*! ***************************************************************
* 函数名：       program_pictureAreaChangeSoundSettings_G6（）
*	areaID：区域的ID号
*	sheader：参考结构体EQPicAreaSoundHeader_G6
*   soundData:语音数据
*
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureAreaChangeSoundSettings_G6(Ouint16 areaID,EQPicAreaSoundHeader_G6 sheader,Ouint8* soundData);
/*! ***************************************************************
* 函数名：       program_pictureAreaDisableSound_G6（）
*	areaID：区域的ID号
*
*
*
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_pictureAreaDisableSound_G6(Ouint16 areaID);
/*! ***************************************************************
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaSetBattleTime_G6(Ouint16 areaID, EQTimeAreaBattle_G6 *header);
/*! ***************************************************************
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaCancleBattleTime_G6(Ouint16 areaID);
/*! ***************************************************************
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaAddContent_G6(Ouint16 areaID,EQtimeAreaData_G56* timeData);
/*! ***************************************************************
* 函数名：       program_timeAreaChangeContent_G6（）
*	areaID：区域的ID号
*   timeData：参考结构体EQtimeAreaData_G56
*
*
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaChangeContent_G6(Ouint16 areaID,EQtimeAreaData_G56* timeData);
/*! ***************************************************************
...
* 
******************************************************************/
LEDEQSDK_API int _CALL_STD program_fontPath_timeAreaAddContent_G6(Ouint16 areaID,EQtimeAreaData_G56* timeData);
/*! ***************************************************************
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaAddAnalogClock_G6(Ouint16 areaID,EQAnalogClockHeader_G56 *header,E_ClockStyle cStyle, ClockColor_G56* cColor);
/*! ***************************************************************
* 函数名：       program_timeAreaChangeAnalogClock_G6(）
* 参数名：
*	areaID：区域ID
*   header: 详情见EQAnalogClockHeader_G56结构体
*   cStyle: 表盘样式，详情见E_ClockStyle
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaChangeAnalogClock_G6(Ouint16 areaID,EQAnalogClockHeader_G56 *header,E_ClockStyle cStyle, ClockColor_G56* cColor);
/*! ***************************************************************
* 函数名：       program_timeAreaChangeDialPic_G6(）
* 参数名：
*	areaID： 区域ID
*   picPath: 表盘图片位置
* 返回值：0 成功， 其他值为错误号
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaChangeDialPic_G6(Ouint16 areaID,Ouint8* picPath);
/*! ***************************************************************
* 函数名：       program_timeAreaRemoveDialPic_G6(）
* 参数名：
*	areaID： 区域ID
*
* 返回值：0 成功， 其他值为错误号
...
*
******************************************************************/
LEDEQSDK_API int _CALL_STD program_timeAreaRemoveDialPic_G6(Ouint16 areaID);
/*! ***************************************************************
...
* EQprogram 结构体是用来回调发送文件所需要参数
******************************************************************/
LEDEQSDK_API int _CALL_STD program_IntegrateProgramFile_G6(EQprogram_G6* program);

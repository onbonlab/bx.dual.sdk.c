...
namespace SDK_CSharp_Form
{
    public class bx_sdk_dual
    {
        ...
        public enum E_DoubleColorPixel_G56:int
        {
            ...
            eDOUBLE_COLOR_PIXTYPE_2 = 2,  //双基色2：G+R
        }
...
******************************************************************/
        [DllImport("bx_sdk_dual.dll", CharSet = CharSet.Unicode)]
        public static extern int bxDual_cmd_uart_ofsReedDirBlock(byte[] com, byte bart, ref GetDirBlock_G56 dirBlock);
    ...
    **************************************************************************************/
        ...
        ******************************************************************/
        [DllImport("bx_sdk_dual.dll", CharSet = CharSet.Unicode)]
        public static extern int bxDual_program_addProgram(ref EQprogramHeader programH); //添加节目句柄
        ...
        *
        * 返回值：0 成功， 其他值为错误号
        * 功 能：时间分区添加内容EQtimeAreaData::fontName == 字体名称
        * 
        ******************************************************************/
        [DllImport("bx_sdk_dual.dll", CharSet = CharSet.Unicode)]
    ...
    **************************************************************************************/
        ...
        ******************************************************************/
        ...
        public static extern int bxDual_dynamicArea_AddAreaTxtDetails_6G(byte[] ip, int nPort, E_ScreenColor_G56 color, byte uAreaId,
            IntPtr oAreaHeader_G6, IntPtr stPageHeader, byte[] fontName, byte[] strAreaTxtContent);
        //动态区文本关联节目: 
...
//RelateProNum = 0 时，关联所有节目，与所有节目一起播放，如果没有节目，则不播放该动态区；
        //			   > 0 时, 指定关联节目，要关联的节目ID存放在RelateProSerial[]中；
        [DllImport("bx_sdk_dual.dll", CharSet = CharSet.Unicode)]
        public static extern int bxDual_dynamicAreaS_AddAreaPic_WithProgram_6G(byte[] ip, int nPort, E_ScreenColor_G56 color, byte uAreaCount, [In] DynamicAreaParams[] pParams, ushort RelateProNum, ushort[] RelateProSerial);
...
******************************************************************/
        [DllImport("bx_sdk_dual.dll", CharSet = CharSet.Unicode)]
        public static extern int bxDual_cmd_uart_ofsDeleteFormatFile(byte[] com, byte bart, short fileNub, byte[] fileName);
...
******************************************************************/
        ...
        */
        [DllImport("bx_sdk_dual.dll", CharSet = CharSet.Unicode)]
        public static extern int bxDual_dynamicArea_InsertSoundIndepend(byte[] ip, ushort nPort, EQSoundDepend_6G stSoundData, byte VoiceFlg, byte StoreFlag);
        ...
        */
        [DllImport("bx_sdk_dual.dll", CharSet = CharSet.Unicode)]
        public static extern int bxDual_dynamicArea_UpdateSoundIndepend(byte[] ip, ushort nPort, [In] EQSoundDepend_6G[] stSoundData, ushort nSoundDataCount, byte StoreFlag);
